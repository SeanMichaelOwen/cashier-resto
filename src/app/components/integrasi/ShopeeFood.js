...
const IntegrationShopeeFood = ({ 
  ...
  const { salesData, performanceData, topProducts } = useMemo(() => {
    try {
      ...
      const salesData = [
        { name: 'Sen', orders: 32, revenue: 800000 },
        { name: 'Sel', orders: 38, revenue: 950000 },
        { name: 'Rab', orders: 45, revenue: 1125000 },
        { name: 'Kam', orders: 42, revenue: 1050000 },
        { name: 'Jum', orders: 55, revenue: 1375000 },
        { name: 'Sab', orders: 68, revenue: 1700000 },
        { name: 'Min', rows: 62, revenue: 1550000 },
      ];
      
      // Data performa platform
      const performanceData = [
        { name: 'Jan', orders: 950, revenue: 23750000 },
        { name: 'Feb', orders: 1100, revenue: 27500000 },
        { name: 'Mar', orders: 1250, revenue: 31250000 },
        { name: 'Apr', orders: 1180, revenue: 29500000 },
        { name: 'Mei', orders: 1400, revenue: 35000000 },
        { name: 'Jun', orders: 1550, revenue: 38750000 },
      ];
      
      // Produk terlaris di ShopeeFood
      const topProducts = products
        .filter(p => p.trending?.shopeefood?.count > 0)
        .sort((a, b) => (b.trending?.shopeefood?.count || 0) - (a.trending?.shopeefood?.count || 0))
        .slice(0, 5)
        .map(product => ({
          ...product,
          shopeefoodOrders: product.trending?.shopeefood?.count || 0,
          shopeefoodRevenue: (product.trending?.shopeefood?.count || 0) * product.price
        }));
      
      return { salesData, performanceData, topProducts };
    } catch (err) {
      console.error('Error processing ShopeeFood data:', err);
      return {
        salesData: [],
        performanceData: [],
        topProducts: []
  ...
  const handleSettingChange = (key, value) => {
    ...
    onUpdateSettings('shopeefood', newSettings);
  };
  ...
  // Produk yang terintegrasi dengan ShopeeFood
  const integratedProducts = products.filter(p => p.integrations?.shopeefood);
    ...
    <div className="bg-white rounded-xl shadow-lg p-6">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
        <div className="flex items-center">
          ...
          <div>
            <h2 className="text-2xl font-bold">Integrasi ShopeeFood</h2>
      ...
      {activeTab === 'overview' && (
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
            <div className="bg-orange-50 p-4 rounded-lg border border-orange-100">
              <div className="flex justify-between">
                <div>
                  <p className="text-sm text-orange-600">Total Pesanan</p>
            ...
            <div className="bg-yellow-50 p-4 rounded-lg border border-yellow-100">
              <div className="flex justify-between">
                <div>
                  ...
                  <h3 className="text-2xl font-bold">4.6/5.0</h3>
              ...
              <div className="h-80">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={salesData}>
                    ...
                    />
                    <Legend />
                    <Bar dataKey="orders" name="orders" fill="#EE4D2D" />
          ...
          {/* Top Products */}
          <div className="bg-white p-4 rounded-lg border border-gray-200 shadow-sm">
            <h3 className="font-medium mb-4">Produk Terlaris di ShopeeFood</h3>
                  ...
                  <tbody>
                    {topProducts.map((product, index) => (
                      <tr key={product.id} className="hover:bg-gray-50">
                        ...
                        <td className="p-3">{product.shopeefoodOrders}</td>
                        <td className="p-3">{formatRupiah(product.shopeefoodRevenue)}</td>
                        <td className="p-3">
                          <div className="flex items-center">
                            {[...Array(5)].map((_, i) => (
                              <FiStar
          ...
          <div className="overflow-x-auto">
            {integratedProducts.length > 0 ? (
                ...
                <tbody>
                  {integratedProducts.map((product) => (
                    <tr key={product.id} className="hover:bg-gray-50">
                      ...
                      <td className="p-3">{product.trending?.shopeefood?.count || 0}</td>
                      <td className="p-3">
                        <span className={`px-2 py-1 rounded-full text-xs ${
                          product.integrations?.shopeefood?.status === 'active' 
                            ? 'bg-green-100 text-green-800' 
                            : 'bg-yellow-100 text-yellow-800'
                        }`}>
                          {product.integrations?.shopeefood?.status === 'active' ? 'Aktif' : 'Tidak Aktif'}
                        </span>
                      </td>
                      <td className="p-3">
                        <div className="flex space-x-2">
              ...
              <div className="text-center py-8 text-gray-500">
                ...
                <p>Belum ada produk yang terintegrasi dengan ShopeeFood</p>
              ...
              <div className="space-y-4">
                ...
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-sm text-gray-500">Update stok di ShopeeFood otomatis</p>
                  </div>
                  <label className="relative inline-flex items-center cursor-pointer">
                    <input 
                      type="checkbox" 
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-sm text-gray-500">Ikuti promosi dari ShopeeFood</p>
                  </div>
                  <label className="relative inline-flex items-center cursor-pointer">
                    <input 
                      type="checkbox" 
                ...
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-sm text-gray-500">Aktifkan pembayaran ShopeePay</p>
                  </div>
                  <label className="relative inline-flex items-center cursor-pointer">
                    <input 
                      type="checkbox" 
                      className="sr-only peer"
                      checked={settings.shopeePayEnabled}
                      onChange={(e) => handleSettingChange('shopeePayEnabled', e.target.checked)}
                    />
                    <div className="w-11 h-6 bg-gray-200 peer-focus:outline-none rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-orange-600"></div>
                  </label>
                </div>
            ...
            <div className="flex">
              ...
              <div>
                ...
                <p className="text-sm text-yellow-700">
                  ...
                  ShopeeFood akan memberikan penalti jika pesanan sering dibatalkan karena stok habis.
                </p>
              </div>
            </div>
          </div>
              ...
              <table className="w-full">
                ...
                <tbody>
                  <tr className="hover:bg-gray-50">
                    ...
                    <td className="p-3">Makanan enak dan harga terjangkau</td>
                  </tr>
                  <tr className="hover:bg-gray-50">
                    <td className="p-3">11 Jun 2023</td>
                    <td className="p-3">#SF-12344</td>
                    ...
                    <td className="p-3">Porsi besar dan pengiriman cepat</td>
                  </tr>
                  <tr className="hover:bg-gray-50">
                    <td className="p-3">10 Jun 2023</td>
                    <td className="p-3">#SF-12343</td>
...
export default IntegrationShopeeFood;